-- ponytail: relax proprietary entity-conflict rejection in csource trigger.
-- ETSI NGSI-LD suite registers exclusive/redirect Context Sources over locally-held
-- entities and expects 201 (routing, not registration, enforces exclusivity). The old
-- RAISE ... ERRCODE=23514 returned 500 and broke every *_exc/*_red setup. We keep the
-- csourceinformation inserts intact and drop only the conflict RAISEs. Idempotent (REPLACE).
								...
								FOR attribName IN SELECT value#>>'{@id}' FROM jsonb_array_elements(infoEntry#>'{https://uri.etsi.org/ngsi-ld/propertyNames}') LOOP
									IF regMode > 1 THEN
										IF entityId IS NOT NULL THEN 
											...
											IF errorFound THEN
					NULL; -- ponytail: entity-conflict RAISE removed for NGSI-LD conformance
											END IF;
