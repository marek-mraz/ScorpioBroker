...
public class EntityOperationsQueryController {
	...
	@Path("/query")
	@POST
	@Counted(value = "batch_entity_queries_total", description = "Total number of entity batch query requests")
	@Timed(value = "batch_entity_query_duration", description = "Duration of entity batch query requests")
	@Timed(value = "batch_entity_queries_concurrent", longTask = true, description = "Number of concurrent entity batch query requests")
	public Uni<RestResponse<Object>> postQuery(HttpServerRequest request, String bodyStr,
		...
		return ctxUni.onItem().transformToUni(context -> {
			try {
				...
				int joinLevel = joinLevelObj == null ? (join == null ? 0 : 1)
