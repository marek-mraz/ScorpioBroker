-- NGSI-LD 5.6.3 Append Attributes: scope handling on append.
-- Previously scope was always replaced with the new value (treated like modifiedAt),
-- ignoring the noOverwrite flag. Per the spec, on a plain append (overwrite) scope is
-- replaced, but with noOverwrite the new scope values are added to the existing scope
-- (union, existing entries first, deduplicated). Only the scope branch changed vs.
-- V20250418.1; the rest of the function is carried verbatim.
...
BEGIN
	FOR attrKey, attrValue IN SELECT * FROM JSONB_EACH(new_entity) LOOP
		...
		ELSIF attrKey = 'https://uri.etsi.org/ngsi-ld/scope' THEN
			-- overwrite (default append) replaces; noOverwrite unions with the existing scope
			IF doReplace OR NOT (old_entity ? attrKey) THEN
				old_entity:= jsonb_set(old_entity, ARRAY[attrKey], attrValue);
			ELSE
				old_entity:= jsonb_set(old_entity, ARRAY[attrKey],
					(old_entity -> attrKey) || COALESCE((
						SELECT jsonb_agg(ni)
						FROM jsonb_array_elements(attrValue) AS ni
						WHERE NOT ((old_entity -> attrKey) @> jsonb_build_array(ni))
					), '[]'::jsonb));
			END IF;
		ELSE
