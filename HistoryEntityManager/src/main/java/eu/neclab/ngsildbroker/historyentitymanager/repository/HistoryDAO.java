...
public class HistoryDAO {
	...
	public Uni<Boolean> createHistoryEntity(CreateHistoryEntityRequest request) {
		...
		return connectionManager.executeQuery(request.getTenant(), sql.toString(), tuple, true).onItem()
				.transformToUni(rows -> {
					...
					for (Entry<String, Object> entry : payload.entrySet()) {
						...
						for (Map<String, Object> attribEntry : entries) {
							...
							if (types != null && types.contains(NGSIConstants.NGSI_LD_GEOPROPERTY)) {
								...
								batchWithLocation.add(Tuple.of(request.getFirstId(), entry.getKey(), new JsonObject(attribEntry), geoLocationVal));
							} else {
	...
	public Uni<Void> batchUpsertHistoryEntity(BatchRequest request) {
		...
		if (!batchType.isEmpty()) {
			...
			tmpList.add(
					connectionManager.executeBatchQuery(request.getTenant(), typeSql.toString(), batchType, true));
		}
		if (!batchNoType.isEmpty()) {
			...
			tmpList.add(connectionManager.executeBatchQuery(request.getTenant(), noTypeSql.toString(), batchNoType,
					true));
		}
		return Uni.combine().all().unis(tmpList).with(l -> l).onItem().transformToUni(l -> {
			...
			if (!batchAttribs.isEmpty()) {
				...
				attribList.add(connectionManager.executeBatchQuery(request.getTenant(), sql.toString(),
						batchAttribs, true));
			}
	...
	public Uni<Void> setDeletedBatchHistoryEntity(BatchRequest request) {
		...
		return connectionManager
				.executeBatchQuery(request.getTenant(), "UPDATE temporalentity SET deletedat=$1 WHERE id=$2", batch,
						false)
				.onItem().transformToUni(t -> Uni.createFrom().voidItem());
	...
	public Uni<Void> deleteHistoryEntity(DeleteHistoryEntityRequest request) {
		String sql = "DELETE FROM " + DBConstants.DBTABLE_TEMPORALENTITY + " WHERE id = $1 RETURNING id";
		return connectionManager.executeQuery(request.getTenant(), sql, Tuple.of(request.getFirstId()), false)
				.onFailure().recoverWithUni(e -> {
					if (e instanceof PgException pge) {
						if (pge.getSqlState().equals(AppConstants.SQL_NOT_FOUND)) {
							return Uni.createFrom().failure(
									new ResponseException(ErrorType.NotFound,
	...
	public Uni<Void> appendToHistoryEntity(AppendHistoryEntityRequest request) {
		...
		return connectionManager.executeQuery(request.getTenant(), sql.toString(), tuple, false).onFailure()
				.recoverWithUni(e -> {
					if (e instanceof PgException pge) {
						if (pge.getSqlState().equals(AppConstants.SQL_NOT_FOUND)) {
							return Uni.createFrom().failure(
					...
					if (!batchWithLocation.isEmpty()) {
						uni2 = connectionManager.executeBatchQuery(request.getTenant(),
								"INSERT INTO " + DBConstants.DBTABLE_TEMPORALENTITY_ATTRIBUTEINSTANCE
										+ " (temporalentity_id, attributeid, data, location) VALUES ($1, $2, $3::jsonb, ST_SetSRID(ST_GeomFromGeoJSON(getGeoJson($4)), 4326))",
								batchWithLocation, false);
					}
					return Uni.combine().all().unis(uni1, uni2).discardItems();
				}).onItem().transformToUni(t -> Uni.createFrom().voidItem()).onFailure().recoverWithUni(e -> {
					if (e instanceof PgException pge) {
	...
	public Uni<Void> updateAttrInstanceInHistoryEntity(UpdateAttrHistoryEntityRequest request) {
		...
		// return client.getConnection().onItem().transformToUni(conn -> {
		return connectionManager.executeQuery(request.getTenant(), "UPDATE "
				...
				Tuple.of(payload, request.getAttribName(), request.getInstanceId(), request.getFirstId()), false)
				.onFailure().recoverWithUni(e -> {
					if (e instanceof PgException pge) {
						if (pge.getSqlState().equals(AppConstants.SQL_NOT_FOUND)) {
							return Uni.createFrom().failure(new ResponseException(ErrorType.NotFound,
					...
					return connectionManager
							.executeQuery(request.getTenant(), "UPDATE " + DBConstants.DBTABLE_TEMPORALENTITY
									...
									Tuple.of(((List<Map<String, String>>) request.getFirstPayload()
	...
	public Uni<Void> deleteAttrInstanceInHistoryEntity(DeleteAttrInstanceHistoryEntityRequest request) {
		return connectionManager
				.executeQuery(request.getTenant(), "DELETE FROM " + DBConstants.DBTABLE_TEMPORALENTITY_ATTRIBUTEINSTANCE
						+ " WHERE attributeid=$1 AND instanceid=$2 AND temporalentity_id=$3",
						Tuple.of(request.getAttribName(), request.getInstanceId(), request.getFirstId()), false)
				.onFailure().recoverWithUni(e -> {
					if (e instanceof PgException pge) {
						if (pge.getSqlState().equals(AppConstants.SQL_NOT_FOUND)) {
							return Uni.createFrom().failure(new ResponseException(ErrorType.NotFound,
	...
	public Uni<Void> setEntityDeleted(BaseRequest request) {
		return connectionManager
				.executeQuery(request.getTenant(), "UPDATE temporalentity SET deletedat=$1 WHERE id=$2", Tuple.of(
						LocalDateTime.ofInstant(Instant.ofEpochMilli(request.getSendTimestamp()), ZoneId.of("Z")),
						request.getFirstId()), false)
				.onItem().transformToUni(t -> Uni.createFrom().voidItem());
	}
	...
	public Uni<Void> setAttributeDeleted(BaseRequest request, String attrType) {
		...
		return connectionManager.executeQuery(request.getTenant(), "INSERT INTO "
				...
				Tuple.of(request.getFirstId(), request.getAttribName(), deletePayload, now, instanceId), false)
				.onFailure().recoverWithUni(e -> {
					if (e instanceof PgException pge) {
						if (pge.getSqlState().equals(AppConstants.SQL_NOT_FOUND)) {
							return Uni.createFrom().failure(new ResponseException(ErrorType.NotFound,
					...
					return connectionManager.executeQuery(request.getTenant(),
							"UPDATE " + DBConstants.DBTABLE_TEMPORALENTITY
									+ " SET modifiedat = $1 WHERE id = $2",
							Tuple.of(now, request.getFirstId()), false);
				}).onItem().transformToUni(t -> Uni.createFrom().voidItem());
	}
