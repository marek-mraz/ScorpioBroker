#!/usr/bin/env bash
# RAM sampler + report for the 5-broker ETSI stack (scorpio-* containers via docker stats).
# Usage:
...
#   dev/mem-monitor.sh report <csv> <outdir>   # memory-report.md (min/median/mean/max) + memory-usage.png
  ...
  sample)
    ...
    while :; do
      docker stats --no-stream --format '{{.Name}},{{.MemUsage}}' 2>/dev/null |
        awk -F'[, ]' -v t="$(date +%s)" '/^scorpio-/{print t","$1","$2}' >> "$CSV"
      sleep 5
    done ;;
  event)
...
import csv, re, statistics, subprocess, sys
try:
    import matplotlib
except ImportError:
...
lines = ["## RAM usage during ETSI run (MiB)", "",
         "| container | samples | min | median | mean | max |",
         "|---|---|---|---|---|---|"]
for name in sorted(series):
    v = [x for _, x in series[name]]
    lines.append(f"| {name} | {len(v)} | {min(v):.0f} | {statistics.median(v):.0f}"
                 f" | {statistics.mean(v):.0f} | {max(v):.0f} |")
...
print(f"wrote {outdir}/memory-report.md + {outdir}/memory-usage.png")
EOF
    ;;
