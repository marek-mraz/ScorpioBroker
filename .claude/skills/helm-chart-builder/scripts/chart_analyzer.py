...
REQUIRED_FILES = [
    ...
    {"path": "README.md", "severity": "medium", "message": "Missing README.md — chart lacks documentation"},
...
TEMPLATE_ANTI_PATTERNS = [
    ...
    },
    {
        "id": "TP008",
        "severity": "high",
        "pattern": r'storageClassName:\s*(?!\{\{).*$',
        "message": "Hardcoded storageClassName — must be configurable via .Values.persistence.storageClass",
        "fix": 'Use conditional block for storageClassName checking .Values.persistence.storageClass',
    },
]
...
def check_crds(chart_dir):
    """Check CRDs directory for templating anti-patterns."""
    findings =[]
    crds_dir = chart_dir / "crds"
    if not crds_dir.exists():
        return findings

    for crd_file in list(crds_dir.glob("*.yaml")) + list(crds_dir.glob("*.yml")):
        content = crd_file.read_text(encoding="utf-8")
        if "{{" in content or "}}" in content:
            findings.append({
                "id": "CRD001",
                "severity": "critical",
                "message": f"CRD file {crd_file.name} contains Helm templates — files in crds/ are NOT templated in Helm 3",
                "fix": "Remove template directives {{ }} from CRDs",
                "file": f"crds/{crd_file.name}",
            })
    return findings
...
def check_templates(chart_dir):
    ...
    for tpl_file in template_files:
        ...
        if "crd-install" in content and "helm.sh/hook" in content:
            findings.append({
                "id": "TP009",
                "severity": "high",
                "message": "crd-install hook is deprecated in Helm 3",
                "fix": "Move CRD files to the crds/ directory without templating",
                "file": str(rel_path),
                "line": "helm.sh/hook: crd-install",
            })
    ...
    # Check for probes
    if ("livenessProbe" not in all_content or "readinessProbe" not in all_content or "startupProbe" not in all_content) and template_files:
        has_deployment = any("Deployment" in f.read_text(encoding="utf-8") for f in template_files if f.suffix in (".yaml", ".yml"))
        if has_deployment:
            findings.append({
                "id": "TP007",
                "severity": "high",
                "message": "Missing one or more health probes (liveness, readiness, startup) — Kubernetes cannot fully manage pod lifecycle",
                "fix": "Always add startupProbe, livenessProbe, and readinessProbe with configurable values",
                "file": "templates/deployment.yaml",
                "line": "(incomplete probes found)",
            })
...
def analyze_chart(chart_dir, output_format="text", security_focus=False):
    ...
    findings.extend(check_crds(chart_dir))
