...
KNOWN_STRUCTURES = {
    ...
    "service": ["type", "port"],
    "ingress": ["enabled"],
    "ingressRoute":["enabled", "host"],
    "resources":[],
    "serviceAccount": ["create", "name"],
    "autoscaling": ["enabled", "minReplicas", "maxReplicas"],
    "persistence":["enabled", "storageClass", "accessMode", "size"],
    "backup":["enabled", "schedule", "timeZone", "ttl"],
}
...
def validate_defaults(entries):
    ...
    for entry in entries:
        ...
        # Check for unquoted float versions that can cause type coercion errors
        if entry["key"] in ("version", "appVersion", "tag") and entry["value_type"] == "float":
            findings.append({
                "severity": "high",
                "category": "defaults",
                "message": f"'{entry['key']}' is a float ({entry['value']}) — YAML type coercion can break versions (e.g., 1.20 becomes 1.2)",
                "fix": f"Quote the string: \"{entry['value']}\"",
                "line": entry["line"],
            })
