...
# -- Startup probe configuration (protects slow-starting containers)
startupProbe:
  httpGet:
    path: /healthz
    port: http
...
backup:
  # -- Enable Velero Schedule creation for disaster recovery
  enabled: false
  ...
  ttl: 720h
  # -- Move CSI snapshot data to backup storage location (Kopia/Restic)
  snapshotMoveData: true
  # -- Volumes to explicitly include or exclude from file-system backup (Kopia/Restic)
  # volumesToBackup: "data-volume"
  # volumesToExclude: "tmp-volume"
  # -- Pre/Post backup hooks (e.g., fsfreeze for application consistency)
  hooks:
    enabled: false
    # preCommand: '["/sbin/fsfreeze", "--freeze", "/data"]'
    # postCommand: '["/sbin/fsfreeze", "--unfreeze", "/data"]'

persistence:
  # -- Enable persistent volume claim
  enabled: false
  # -- Storage class name (leave empty for cluster default, "-" to disable dynamic provisioning)
  storageClass: ""
  # -- PVC Access Mode
...
sealedSecret:
  # -- Enable SealedSecret instead of plain Secret
  enabled: false
  # -- Encrypted data for SealedSecret (generate with kubeseal)
  encryptedData: {}
```
...
### 5. Type Coercion and Quoting Strings

```yaml
# BAD — YAML interprets 1.2 as a float, and large numbers as scientific notation.
appVersion: 1.2
port: 80

# GOOD — Quote strings to avoid Helm/Kubernetes type conversion errors.
appVersion: "1.2"
port: 80
```
...
### 7. Deleting Default Keys

To remove a key from a parent or default `values.yaml`, override it with `null`.
