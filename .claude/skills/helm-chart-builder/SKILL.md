...
| `/helm:flow` | Scaffold initContainers for strict service dependency flow and startup ordering |
| `/helm:velero` | Integrate GitOps-safe Velero backup schedules, volume annotations, and hooks into the chart |
...
2. **Scaffold chart structure**
   *Always create a Full Production Chart. Do not use minimal templates.*
   ...
   │   ├── pvc.yaml            # PersistentVolumeClaim (if persistence enabled)
   │   ├── schedule.yaml       # Velero Backup Schedule (if backups enabled)
...
3. **Apply Chart.yaml best practices**
   ...
   ├── Prefer OCI-based registries (oci://...) for dependencies when available
   └── Do NOT use Bitnami images or Helm charts (often deprecated), except bitnami-labs/sealed-secrets

   HOOKS
   ├── Use "helm.sh/hook" annotations to tap into the release lifecycle (pre-install, post-install, test, etc.)
   ├── Set "helm.sh/hook-weight" (as a string) to control execution order
   └── Set "helm.sh/hook-delete-policy" (e.g., "hook-succeeded", "before-hook-creation") to clean up hook resources
   ```
...
### Advanced Backends
Helm uses Secrets for release information by default. For large releases (>1MB), consider the SQL backend:
```bash
...
### `/helm:velero` — Disaster Recovery & Backups

1. **Enable GitOps-safe Velero Backups**
   - Inject Velero backup Schedule templates natively.
   - Utilize ArgoCD annotations (`argocd.argoproj.io/compare-options: IgnoreExtraneous`, `argocd.argoproj.io/sync-options: Delete=false,Prune=false`) to prevent extraneous resource pruning of generated Backup objects.
   - Advise explicitly against using `useOwnerReferencesInBackup` in GitOps environments to prevent cascading garbage collection of backup history.
   - Use `snapshotMoveData: true` in Backup/Schedule specs to instruct the Velero data mover (Kopia) to move CSI snapshot data to the remote backup storage location, protecting against local storage loss.

2. **Pod-level Configuration**
   - Integrate `backup.velero.io/backup-volumes` and `backup.velero.io/backup-volumes-excludes` annotations in the Pod template to ensure Restic/Kopia picks up the correct state.
   - Insert Velero hooks (`pre.hook.backup.velero.io/command`, `post.hook.backup.velero.io/command`) where application-consistent backups are needed (e.g., executing `fsfreeze` or database checkpoints).
   - Insert Velero Restore hooks (`init.hook.restore.velero.io/command` or `post.hook.restore.velero.io/command`) if database initialization or state cleaning is required before the app resumes.

3. **Restoration Readiness**
   - Suggest proper `ResourceModifiers` (as a ConfigMap) for stripping out GitOps tracking tags (`app.kubernetes.io/instance`, `argocd.argoproj.io/tracking-id`) upon restore.
...
- **Missing health probes** → Add `startupProbe`, `livenessProbe`, and `readinessProbe`. Kubernetes needs to manage pod lifecycles comprehensively.
- **Missing app.kubernetes.io labels** → Add via _helpers.tpl. Required for proper resource tracking.
- **Race conditions / No InitContainers** → Use `initContainers` to wait for databases or service dependencies.
- **CRD Templating** → Ensure `crds/` folder files do not contain `{{ }}` templates as they are skipped by Helm engine. Do not use the deprecated `crd-install` hook.
- **Bitnami Usage** → Warn against using Bitnami images/charts unless it's `sealed-secrets`.
