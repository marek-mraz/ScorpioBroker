-- NGSI-LD Update Attributes (PATCH /entities/{id}/attrs): behaves like Append (merges types,
-- adds non-present attributes, replaces present ones) with ONE exception — `scope` is only
-- replaced when already present and is NEVER added if absent (the caller reports it as
-- notUpdated -> 207). Identical to ngsild_update_entity apart from the scope branch.
CREATE OR REPLACE FUNCTION public.ngsild_update_existing_attribs(IN old_entity jsonb,IN new_entity jsonb,IN doreplace boolean)
    RETURNS jsonb
    LANGUAGE 'plpgsql'
    VOLATILE
    PARALLEL SAFE
...
BEGIN
	FOR attrKey, attrValue IN SELECT * FROM JSONB_EACH(new_entity) LOOP
		...
		ELSIF attrKey = 'https://uri.etsi.org/ngsi-ld/scope' THEN
			-- update only replaces an existing scope; never adds one when absent
			IF old_entity ? attrKey THEN
				old_entity:= jsonb_set(old_entity, ARRAY[attrKey], attrValue);
			END IF;
		ELSE
